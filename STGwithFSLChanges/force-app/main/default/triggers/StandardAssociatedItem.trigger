trigger StandardAssociatedItem on Associated_Item__c (before update, after update, before insert, after insert, before delete, after delete) 
{
    ...
    if(Trigger.isAfter)
    {
        ...
        Set<ID> orderitemids = new Set<ID>();
				//shawks FSL Phase 1: Update to use standard Work Order (WorkOrder) object instead of SONIFI WO (Work_Order__c)
        ...
        if(Trigger.isInsert)
        {
    	    ...
    	    {
    			//shawks FSL Phase 1: Update to use standard Work Order object reference instead of SONIFI WO
					//workorderids.add(recaitem.Work_Order__c);
					workorderids.add(recaitem.WorkOrder__c);
        ...
        {	
        	...
        	//shawks FSL Phase 1: Update to use standard Work Order object and references instead of SONIFI WO
					//List<Work_Order__c> lstrelatedwo = New List<Work_Order__c>();
					List<WorkOrder> lstrelatedwo = New List<WorkOrder>();
        	for(Associated_Item__c recaitem : trigger.old)
    	    {
    	    	//shawks FSL Phase 1: Use new reference and comment out old
						//if(recaitem.name.contains('Product Parent') && recaitem.work_order__c != null)
    	    	//{ woids.add(recaitem.work_order__c);
						if(recaitem.name.contains('Product Parent') && recaitem.workorder__c != null)
    	    	{ woids.add(recaitem.workorder__c);    
